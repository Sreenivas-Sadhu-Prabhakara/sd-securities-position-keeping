...
| POST | `/v1/securities-position-maintenance-agreement/initiate` | Initiate |
| GET | `/v1/securities-position-maintenance-agreement` | Retrieve (list) |
| GET | `/v1/securities-position-maintenance-agreement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/securities-position-maintenance-agreement/{crId}/update` | Update |
| PUT | `/v1/securities-position-maintenance-agreement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/securities-position-maintenance-agreement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-securities-position-keeping:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-securities-position-keeping ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-securities-position-keeping` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
